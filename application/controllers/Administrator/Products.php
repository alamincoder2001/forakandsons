...
class Products extends CI_Controller {
    ...
    public function getProducts(){
        ...
        $products = $this->db->query("
            ...
            left join tbl_unit u on u.Unit_SlNo = p.Unit_ID
            where p.status = 'a' and p.Product_branchid=? 
            $clauses 
    ...
    left join tbl_brand b on b.brand_SiNo = p.brand
    left join tbl_unit u on u.Unit_SlNo = p.Unit_ID
    where p.status = 'a' and p.is_service = 'false'")->result();
    ...
    $this->db->where('product_id', $val->Product_SlNo);
    $this->db->where('branch_id', $val->Product_branchid);
    $this->db->update('tbl_currentinventory', $data);
    }
