...
class Sales extends CI_Controller {
    ...
    public function getCustomersByArea(){
        $data = json_decode($this->input->raw_input_stream);
        $areas = $this->db->query("select c.*,a.District_Name, concat(c.Customer_Code, ' - ', c.Customer_Name, ' - ', c.owner_name, ' - ', c.Customer_Mobile) as display_name
        from tbl_customer c 
        left join tbl_district a on a.District_SlNo = c.area_ID
        where  c.area_ID=?  and c.Customer_brunchid=?  and c.status='a' ",[$data->area_id,$this->session->userdata("BRANCHid")])->result(); 
    ...
    public function addSales(){
        ...
        try{
            ...
            $sales = array(
                ...
                'damage_amount'=>$data->sales->damageAmount
            );
            ...
            foreach($data->cart as $cartProduct){
                $saleDetails = array(
                    ...
                    'SaleDetails_Discount'      => $cartProduct->discountPercent,
    ...
    public function getSaleDetails(){
        ...
        if(isset($data->searchType) && $data->searchType=='quantity' && $data->productId == ''){
            
            $saleDetails = $this->db->query("
            select 
                sd.*,
                sum(SaleDetails_TotalQuantity) as SaleDetails_TotalQuantity,
                p.Product_Name,
...
//    }
    ...
    public function getSummaryTransactions(){
        $data = json_decode($this->input->raw_input_stream);
        $branchId = $this->session->userdata("BRANCHid");
        $summary  = $this->db->query("select acc.Acc_Name,
        (select ifnull(sum(In_Amount),0) from tbl_cashtransaction where Acc_SlID= acc.Acc_SlNo and Tr_Type='In Cash' and Tr_branchid='$branchId' and status='a' ) as receivedAmount ,
        (select ifnull(sum(Out_Amount),0) from tbl_cashtransaction where Acc_SlID= acc.Acc_SlNo and Tr_Type='Out Cash' and Tr_branchid='$branchId' and status='a' ) as   paymentAmount
        from tbl_account acc 
        left join tbl_cashtransaction tran on  tran.Acc_SlID = acc.Acc_SlNo
        where acc.branch_id = '$branchId' and acc.status='a'
        group by acc.Acc_SlNo
        ")->result();
    ...
    public function getProfitLoss(){
        ...
        if($data->productId != null && $data->productId != ''){
            $productClause = " and sd.Product_IDNo = $data->productId ";
        }
