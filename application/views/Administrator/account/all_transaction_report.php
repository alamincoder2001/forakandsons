...
<div id="cashTransactionReport">
    <div class="row" style="border-bottom: 1px solid #ccc;padding: 3px 0;">
        <div class="col-md-12">
            <form class="form-inline" id="searchForm" @submit.prevent="getTransactions">
                ...
                <div class="form-group" style="display:none;" v-bind:style="{display: accounts.length > 0 && filter.transactionType  != 'summary'   ? '' : 'none'}">
    ...
    <div class="row" style="display:none;" v-bind:style="{display: (transHeads.length >  0 && filter.transactionType  == 'summary') ? '' : 'none'}">
        ...
        <div class="col-md-12">
            <div class="table-responsive" id="printContent">
                <table class="table table-bordered table-condensed">
                    ...
                    <tbody>
                        <tr v-for="transaction in transHeads">
                        
                           
                            <td>{{ transaction.Acc_Name }}</td>
                           
                            <td style="text-align:right;">{{ transaction.paymentAmount }}</td>
                            <td style="text-align:right;">{{ transaction.receivedAmount }}</td>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td style="text-align:right;font-weight:bold;">{{ transHeads.reduce((p, c) => { return p + parseFloat(c.paymentAmount) }, 0) }}</td>
                            <td style="text-align:right;font-weight:bold;">{{ transHeads.reduce((p, c) => { return p + parseFloat(c.receivedAmount) }, 0) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
    ...
    <div class="row" style="display:none;" v-bind:style="{display: (transactions.length >  0 && filter.transactionType  != 'summary') ? '' : 'none'}">
        <div class="col-md-12" style="margin-top:15px;margin-bottom:15px;">
            <a href="" @click.prevent="print"><i class="fa fa-print"></i> Print</a>
        </div>
        <div class="col-md-12">
...
<script>
    ...
    new Vue({
        ...
        methods: {
            ...
            getTransactions()  { 
                if(this.filter.transactionType =='summary'){
                    axios.post('/get_summary_transactions', this.filter).then(res => {
                    this.transHeads = res.data;
                    })
                }else{
