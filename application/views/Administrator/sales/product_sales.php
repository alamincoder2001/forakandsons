				...
				<div class="widget-main">
					...
					<div class="row">
						<div class="col-sm-5">
							...
							<div class="form-group">
								<label class="col-sm-4 control-label no-padding-right"> By Route </label>
								<div class="col-sm-7">
									<v-select v-bind:options="areas" label="District_Name" v-model="selectedArea" v-on:input="areaOnChange"></v-select>
								</div>
								<div class="col-sm-1" style="padding: 0;">
									<a href="<?= base_url('area') ?>" class="btn btn-xs btn-danger" style="height: 25px; border: 0; width: 27px; margin-left: -10px;" target="_blank" title="Add New Area"><i class="fa fa-plus" aria-hidden="true" style="margin-top: 5px;"></i></a>
								</div>
							</div>
						...
						<div class="col-sm-5">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input type="number" id="salesRate" placeholder="Rate" step="0.01" class="form-control" v-model="selectedProduct.Product_SellingPrice" v-on:input="productTotal" readonly />
									</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<div class="col-sm-4">
										<input type="number" step="0.01" id="discountPercent" placeholder="Percent" class="form-control" ref="discountPercent" v-model="selectedProduct.discountPercent" v-on:input="productTotal" autocomplete="off" required />
									</div>
									<div class="col-sm-5">
										<input type="number" step="0.01" id="discountAmount" placeholder="Amount" class="form-control" ref="discountAmount" v-model="selectedProduct.discountAmount" v-on:input="productTotal" autocomplete="off" required />
									</div>
								</div>
								<div class="form-group">
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-sm-12 control-label no-padding-right">Discount Percent</label>
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-sm-12 control-label no-padding-right">Damage Amount : </label>
												<div class="col-sm-12">
													<input type="number" class="form-control" v-model="sales.damageAmount" v-on:input="calculateTotal" />
												</div>
											</div>
										</td>
									</tr>
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-sm-12 control-label no-padding-right">SBS Comission</label>
												<div class="col-sm-12">
									...
									<tr>
										<td>
											<div class="form-group">
												...
												<div class="col-sm-12">
													<input type="number" id="paid" class="form-control" v-model="sales.paid" v-on:input="calculateTotalDue" v-bind:disabled="selectedCustomer.Customer_Type == 'G' ? true : false" />
												</div>
											</div>
										</td>
									</tr>
		...
		methods: {
			...
			async areaOnChange() {
				if (this.selectedArea != null || this.selectedArea.District_SlNo != '0') {
					await axios.post('/get_customers_by_area', {
						area_id: this.selectedArea.District_SlNo
					}).then(res => {
			...
			productTotal() {
				...
				// product wish discount count
				if (event.target.id == 'discountPercent') {
					this.selectedProduct.discountAmount = ((parseFloat(total) * parseFloat(this.selectedProduct.discountPercent)) / 100).toFixed(2);
				} else {
					this.selectedProduct.discountPercent = (parseFloat(this.selectedProduct.discountAmount) / parseFloat(total) * 100).toFixed(2);
				}

				this.selectedProduct.total = total - this.selectedProduct.discountAmount;
			...
			addToCart() {
				let product = {
					...
					discountPercent: this.selectedProduct.discountPercent,
				...
				// if(product.quantity > this.productStock && this.sales.isService == 'false' && this.branchId != 6){
				...
				this.sales.paid = this.cart.reduce((prev, curr) => {
			...
			calculateTotal() {
				...
				this.sales.total = ((parseFloat(this.sales.subTotal) - parseFloat(this.sales.damageAmount) + parseFloat(this.sales.vat) - parseFloat(this.sales.transportCost)) - parseFloat(this.sales.discount)).toFixed(2);
				...
				axios.post('/get_sales', {
					...
					r.saleDetails.forEach(product => {
						let cartProduct = {
							...
							discountPercent: product.SaleDetails_Discount,
